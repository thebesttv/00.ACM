...
struct ST{
  ...
  void init(){
    int n4 = n*4;
    Node *p = new Node; p->s = 0; p->l = p->r = p;
    FORR(i,1,n4) cur[i] = p;
    head[0] = p;
  }
  void insert(int i, int x){
    head[i] = update(1, 1, n, x);
  }
  Node* update(int u, int l, int r, int x){
    Node *p = new Node;
    if(l == r){
      p->s = cur[u]->s + 1;
      p->l = p->r = nullptr;
    ...
    int m = (l+r)>>1;
    if(x<=m){
      p->l = cur[u<<1] = update(u<<1, l, m, x);
      p->r = cur[u<<1|1];
    }else{
      p->l = cur[u<<1];
      p->r = cur[u<<1|1] = update(u<<1|1, m+1, r, x);
    }
    p->s = p->l->s + p->r->s;
    return cur[u] = p;
  }
  ...
  int query(Node *u, Node *v, int l, int r, int k){
    ...
    if(k <= ls){
      ...
      return query(u->r, v->r, m+1, r, k - ls);
    }
  }
}sg;
...
int main(void){
  ...
  sort(v,v+n); int cnt = unique(v,v+n) - v;
  sg.n = cnt; sg.init();
  FORR(i,1,n) sg.insert(i, lower_bound(v,v+cnt,a[i]) - v + 1);
