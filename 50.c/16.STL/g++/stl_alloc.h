...
// 第一级 allocator: __malloc_alloc_template
#if 0
// 为何 if 0 ?
#   include <new>
...
static void * allocate(size_t n)
{
    ...
    if (0 == result) result = oom_malloc(n);    // when out of memory
    return result;
}
...
static void * reallocate(void *p, size_t /* old_sz */, size_t new_sz)
{
    ...
    if (0 == result) result = oom_realloc(p, new_sz);   // when out of memory
    return result;
}

// 不能使用 set_new_handler(), 从而仿真一个 set_malloc_handler()
...
template<class T, class Alloc>  // 对第一、二级 allocator 的封装
class simple_alloc {
...
# ifdef __USE_MALLOC    // 仅使用第一级 allocator

typedef malloc_alloc alloc; // 定义为 `alloc`
...
# ifndef __SUNPRO_CC
    enum {__ALIGN = 8};         // 小型区块上界
    enum {__MAX_BYTES = 128};   // 下界
    enum {__NFREELISTS = __MAX_BYTES/__ALIGN};  // free-list 个数
# endif
  static size_t ROUND_UP(size_t bytes) {    // 将 bytes 上调至 __ALIGN 的倍数
...
# endif
  ...
  static char *start_free;  // 内存池起、止位置
...
#       endif
    ...
    *my_free_list = result -> free_list_link;
    return (result);
    // 链表删除
    // 从 free list 中拔出 result
    // 1. my_free_list = &free_list[n]
    // 2. result = free_list[n]
    // 3. free_list[n] = result->next
  };
...
typedef __default_alloc_template<__NODE_ALLOCATOR_THREADS, 0> alloc;    // 定义为 `alloc`
...
/* We assume that size is properly aligned. 假设 size 已经是 8 的倍数      */
...
__default_alloc_template<threads, inst>::chunk_alloc(size_t size, int& nobjs)
{
    ...
    if (bytes_left >= total_bytes) {
        ...
        return(result);
    } else if (bytes_left >= size) { // 至少能分配一块
        nobjs = bytes_left/size;    // 仅使用内存池剩余空间（可能只分配一块）
        ...
        // Try to make use of the left-over piece. (精打细算.jpg)
        if (bytes_left > 0) {
            // 插入 free list
            obj * __VOLATILE * my_free_list =
...
/* Returns an object of size n, and optionally adds to size n free list.*/
/* We assume that n is properly aligned. 假设 n 已经是 8 的倍数            */
/* We hold the allocation lock.                                         */
template <bool threads, int inst>
void* __default_alloc_template<threads, inst>::refill(size_t n)
{
    // 默认新增 20 个节点, 但内存不足时可能变小
    int nobjs = 20;
    ...
    // 若只获得一个节点，则直接返回，free list 无新增节点
    if (1 == nobjs) return(chunk);
    my_free_list = free_list + FREELIST_INDEX(n);

    /* Build free list in chunk */
      result = (obj *)chunk;    // 最后返回给 client 的一块
      // 新增 nobjs - 1 块大小为 n 的节点
      // 先令 free list 指向第  块，再将剩下块串起来
      *my_free_list = next_obj = (obj *)(chunk + n);    // 第 2 块开头
      for (i = 1; /* i < nobjs (下面以给出，无必要) */ ; i++) {
