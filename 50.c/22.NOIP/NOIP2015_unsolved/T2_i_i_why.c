...
/*
  ...
  解题报告：
    问题可以转换为求出一个有向图中的最小环。
    一开始没注意每个点只能伸出一条出边，于是方向就错了。
    后来发现每个点只能有一条出边，那么图就不会很复杂。
    对于30%的数据，直接Floyd再求每个强连通的点对中的最小距离和即可。
    然后想到，只要固定起点，那么它的路径就是定的，就可以对每个点dfs，
  正解：
    这道题的正解一定不是并查集什么的。。。
...
void euler(int u, int step){  //好像不应该叫这名字。。。
...
void merge(int x, int y){
  ...
  if(px==py){
    ansMin=min(ansMin,dis[x]+dis[y]+1); //???
  }else{
    dis[x]=dis[y]+1;  //把x接到y上
    //p[x]=y; //???
    p[px]=py;
  }
}
...
/*
const int MAX=200020;
int n,to[MAX],vis[MAX],stp[MAX];
int kase=0;
int ansMin=1<<30;

void dfs(int cur, int step){
  if(vis[cur]==kase) ansMin=min(ansMin,step-stp[cur]);
  else {vis[cur]=kase; stp[cur]=step; dfs(to[cur],step+1);}
}

int main(void){
  scanf("%d",&n);
  FORR(i,1,n) scanf("%d",&to[i]);
  FORR(i,1,n) if(!vis[i]){
    kase++; dfs(i,0);
  }
  printf("%d\n",ansMin);
