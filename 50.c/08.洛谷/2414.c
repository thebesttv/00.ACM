...
int main(void){
  ...
  while(i<n){
    ...
    while(Q[i].y==Y){
      int t=ac.getNode(Q[i].x);
      ans[Q[i].num] = st.sum( ft.node[t].l, ft.node[t].r );
      i++;
    }
  }
  */
  st.n=ft.cnt; char *s=t1;
  int i=0,ch,u=0,cnt=0;
  while(ch=*s++){
    if(ch=='P'){
      cnt++;
      while(i<n && Q[i].y<cnt) i++;
      while(i<n && Q[i].y==cnt){
        int t=ac.getNode(Q[i].x);
        ans[Q[i].num] = st.sum( ft.node[t].l, ft.node[t].r );
        i++;
