...
struct ST{
  ...
  void pushdown(int u, int l, int r, int m){
    if(sv[u] != 1){
      v[u<<1] = v[u<<1] * sv[u] %MOD;
      v[u<<1|1] = v[u<<1|1] * sv[u] %MOD;
      sv[u<<1] = sv[u<<1] * sv[u] %MOD;
      sv[u<<1|1] = sv[u<<1|1] * sv[u] %MOD;
      sv[u] = 1;
    }
  }
  void pushup(int u){
  ...
  LL query(int u, int cl, int cr, int ql, int qr){
    ...
    if(ql <= m) s += query(u<<1, cl, m, ql, qr);
    if(m+1 <= qr) s += query(u<<1|1, m+1, cr, ql, qr);
    return s%MOD;
  }
}sg;
